...
이 폴더는 **스마트 군수물자 관리 앱**의 DB 설계 및 초기 구현을 담당하는 부분이다.
...
```
Firestore
 ├─ weapons            화기 마스터 (k1 / k2 / k2c1, 3개 문서 고정. M16A1은 2026-06-23부로 범위 제외)
...
- **문서ID(`weapons.code`)와 `images.weaponClass`, `detectionRecords.confirmedDetections[].class`는 모두 YOLO 모델의 클래스명(`data/data_no_m16.yaml`의 `names`)과 소문자로 정확히 일치시킨다.** 현재 대상은 `k1`, `k2`, `k2c1` 3종이다 (M16A1/`m16`은 2026-06-23부로 학습 데이터 품질 문제 등으로 범위에서 제외 결정됨, 팀원 쪽 `models/firearms_yolo_no_m16/best.pt`와 일치).
  - 참고: 이전에는 `m16`(M16A1)을 포함한 4종으로 설계됐었다. 제외 결정 이후 `weapons` 컬렉션의 `m16` 문서는 삭제했다. 추후 다시 포함하기로 결정되면 `seed_weapons.py`에 항목을 다시 추가하고 재실행하면 된다.
...
- 대상 화기는 `k1`, `k2`, `k2c1` 3종이다. M16A1(`m16`)은 2026-06-23부로 범위에서 제외됐다(팀원 쪽 `data/data_no_m16.yaml`, `models/firearms_yolo_no_m16/best.pt` 참고). `weapons` 마스터 데이터 중 `overallLengthMm`, `weightKg` 등 세부 제원은 아직 공식 출처로 검증되지 않아 `null`로 비워두고 `specsVerified: false`로 표시해 두었다. 임의로 수치를 채우지 말고 검증 후 업데이트할 것.
